...
{
	// Try to use plugin content directory first
	IPluginManager& PluginManager = IPluginManager::Get();
	TSharedPtr<IPlugin> Plugin = PluginManager.FindPlugin(TEXT("ShortStory"));
	if (Plugin.IsValid())
	{
		return FPaths::Combine(Plugin->GetContentDir(), TEXT("Stories"));
	}

	// Fallback to project directory if plugin not found
	UE_LOG(LogShortStory, Warning, TEXT("ShortStory plugin not found, using project content directory as fallback"));
	return FPaths::Combine(FPaths::ProjectContentDir(), TEXT("Stories"));
}
